...
`prefer-alphabetical-scripts` を有効にすることで、`scripts` がアルファベット順でない場合にエラーにすることができます。
...
`prefer-absolute-version-dependencies` と `prefer-absolute-version-devDependencies` を有効にすることで、パッケージのバージョンに `~` や `^` がある場合にエラーにすることができます。
...
プロジェクト内の決まり事は、それを担保するために可能であれば仕組み化できると嬉しいなと改めて思いました。
