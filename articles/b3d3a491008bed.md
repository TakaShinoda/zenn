...
`prefer-alphabetical-scripts` を有効化することで、`scripts` がアルファベット順でない場合にエラーにすることができます。
...
`prefer-no-version-zero-dependencies` と `prefer-no-version-zero-devDependencies` を有効化することで、メジャーバージョンが `0` 系であるパッケージがある場合にエラーにすることができます。
...
`prefer-absolute-version-dependencies` と `prefer-absolute-version-devDependencies` を有効化することで、パッケージのバージョンに `~` や `^` がある場合にエラーにできます。
